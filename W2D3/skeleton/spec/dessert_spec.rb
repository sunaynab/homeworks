...
describe Dessert do
  let(:chef) { double("chef", name: "Sara") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cookie", "a dozen", chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["flour", "butter", "sugar", "salt", "vanilla"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Sara the Great Baker")
      expect(cake.serve).to eq("Chef Sara the Great Baker has made 2 cakes!")
    end
